...
/***/ function(module, exports, __webpack_require__) {
	...
	var _safeAccess = __webpack_require__(300);

	var _safeAccess2 = _interopRequireDefault(_safeAccess);

	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

	var _marked = [traverse].map(regeneratorRuntime.mark);
	...
	function traverse(root) {
	  var isExclusive = arguments.length <= 1 || arguments[1] === undefined ? false : arguments[1];

	  var treeWalker, node, nextNode, attrs, i, attr, match, _match, length;

	  return regeneratorRuntime.wrap(function traverse$(_context) {
	    while (1) {
	      switch (_context.prev = _context.next) {
	        case 0:
	          treeWalker = document.createTreeWalker(root, NodeFilter.SHOW_ELEMENT | NodeFilter.SHOW_TEXT, {
	          ...
	          node = isExclusive ? treeWalker.nextNode() : treeWalker.currentNode;
	          nextNode = void 0;
	        ...
	        case 3:
	          ...
	          _context.t0 = node.nodeType;
	          _context.next = _context.t0 === Node.ELEMENT_NODE ? 7 : _context.t0 === Node.TEXT_NODE ? 25 : 33;
	          break;
	        ...
	        case 17:
	          match = attr.value.match(EXPR_REGEX);
	        ...
	        case 25:
	          _match = node.textContent.match(EXPR_REGEX);
	          ...
	          if (_match.index > 0) {
	            node = node.splitText(_match.index);
	            // forward the walker
	            nextNode = node;
	          }
	...
	function loop(scope, expr, fn) {
	  var match = expr.match(LOOP_REGEX);
	  if (match) {
	    (function () {
	      var iterVar = match[1];
	      var collection = (0, _safeAccess2.default)(scope, match[2]);
	...
	function interpolate(_ref, scope) {
	  var op = _ref.op;
	  var node = _ref.node;
	  var expr = _ref.expr;
	  ...
	  switch (op) {
	    case EXPR:
	      {
	        var val = (0, _safeAccess2.default)(scope, expr);
	        if (val !== UNDEFINED) {
	    ...
	    case CONDITION:
	      {
	        var condition = (0, _safeAccess2.default)(scope, expr);
	        if (condition) {
	    ...
	    case LOOP:
	      {
	        var _ret2 = function () {
	          ...
	          loop(scope, expr, function (itemScope) {
	            ...
	            var perItemFrag = itemFrag.cloneNode(true);
	...
	function sink(fn) {
	  return function ($el) {
	    ...
	    var older = $el.style.visibility;
	    $el.style.visibility = 'hidden';
	    var sum = fn.apply(this, arguments);
	...
	function render($el, scope, isExclusive) {
	  return Array.from(traverse($el, isExclusive)).reduce(function (sum, op) {
